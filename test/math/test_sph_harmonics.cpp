...
auto test_ang_grad_phi() -> void {
    ...
    // ---------- m = 0: (1/sinθ)·∂Y_l0/∂φ = 0 for any l ----------
    {
        ...
        RealSphericalHarmonicsEngine eng{std::span<const double>(&theta, 1), std::span<const double>(&phi, 1), 4};
        for (int l = 0; l <= 4; ++l) {
            auto gp = eng.get_ang_grad_phi(l, 0);
            check(near(gp[0], 0.0, eps),
                  std::format("get_ang_grad_phi({},0) = 0", l));
        }
    }
    ...
    // ---------- m > 0: (1/sinθ)·∂Y_{lm}/∂φ = -m/sinθ · Y_{l,-m} ----------
    {
        ...
        for (int l = 1; l <= 4; ++l) {
            for (int m = 1; m <= l; ++m) {
                auto gp = eng.get_ang_grad_phi(l, m);
                ...
                check(near(gp[0], expected, 1e-12),
                      std::format("ang_grad_phi({},{}) = -{}/sinθ · Y({},{})", l, m, m, l, -m));
            }
        }
    }
    ...
    // ---------- m < 0: (1/sinθ)·∂Y_{lm}/∂φ = |m|/sinθ · Y_{l,|m|} ----------
    {
        ...
        for (int l = 1; l <= 4; ++l) {
            for (int m = -l; m <= -1; ++m) {
                ...
                check(near(gp[0], expected, 1e-12),
                      std::format("ang_grad_phi({},{}) = {}/sinθ · Y({},{})", l, m, m_abs, l, m_abs));
            }
        }
    }
    ...
    // ---------- Finite-difference check ----------
    {
        ...
        for (int l = 0; l <= 4; ++l) {
            for (int m = -l; m <= l; ++m) {
                ...
                double analytic = eng_ref.get_ang_grad_phi(l, m)[0];
                // fd ≈ sinθ · analytic  →  fd/sinθ ≈ analytic
                check(near(fd, sin_t * analytic, 1e-6),
                      std::format("finite diff dY({},{})/dphi = sinθ·ang_grad_phi", l, m));
            }
        }
    }
    ...
    // ---------- theta = 0 edge: |m|=1 finite, others zero ----------
    {
        ...
        for (int l = 0; l <= 4; ++l) {
            auto gp = eng.get_ang_grad_phi(l, 0);
            check(near(gp[0], 0.0, eps),
                  std::format("ang_grad_phi({},0) = 0 at θ=0", l));
        }
        ...
        // l=1, m=±1: known analytic values at θ=0
        {
            ...
            auto gp11 = eng.get_ang_grad_phi(1, 1);
            check(near(gp11[0], expected_11, 1e-12),
                  "ang_grad_phi(1,1) at θ=0 matches analytic limit");

            double expected_1m1 = -std::sqrt(3.0 / (4.0 * std::numbers::pi)) * std::cos(phi_val);
            auto gp1m1 = eng.get_ang_grad_phi(1, -1);
        ...
        for (int l = 2; l <= 4; ++l) {
            for (int m = -l; m <= l; ++m) {
                ...
                check(near(gp[0], 0.0, 1e-14),
                      std::format("ang_grad_phi({},{}) = 0 at θ=0 (|m|>=2)", l, m));
            }
        }
    }
    ...
    // ---------- Mode=None throws ----------
    {
        ...
        RealSphericalHarmonicsEngine eng{std::span<const double>(&theta, 1), std::span<const double>(&phi, 1),
                                   0, RealSphericalHarmonicsEngine::CacheMode::None};
        try {
            eng.get_ang_grad_phi(0, 0);
            check(false, "get_ang_grad_phi should throw in CacheMode::None");
        } catch (const std::runtime_error&) {
            check(true, "get_ang_grad_phi throws in CacheMode::None");
        }
    }
    ...
    // ---------- l > l_max_resident throws ----------
    {
        ...
        RealSphericalHarmonicsEngine eng{std::span<const double>(&theta, 1), std::span<const double>(&phi, 1), 2};
        try {
            eng.get_ang_grad_phi(3, 0);
            check(false, "get_ang_grad_phi should throw when l > l_max_resident");
        } catch (const std::runtime_error&) {
            check(true, "get_ang_grad_phi throws when l > l_max_resident");
        }
    }
}

auto test_ang_grad_theta() -> void {
    std::println("\n=== get_ang_grad_theta() ===");
    ...
    // ---------- m = 0: ∂Y_l0/∂θ = sqrt(l(l+1)) · Q_l^1 (l=0 → 0) ----------
    {
        ...
        // l=0: ∂Y_00/∂θ = 0
        auto gt00 = eng.get_ang_grad_theta(0, 0);
        check(near(gt00[0], 0.0, eps),
              "get_ang_grad_theta(0,0) = 0");

        // l=1: Y_10 = sqrt(3/(4π))·cosθ → ∂Y_10/∂θ = -sqrt(3/(4π))·sinθ
        double sin_t = std::sin(theta);
        double expected_10 = -std::sqrt(3.0 / (4.0 * std::numbers::pi)) * sin_t;
        auto gt10 = eng.get_ang_grad_theta(1, 0);
        check(near(gt10[0], expected_10, eps),
              "get_ang_grad_theta(1,0) matches analytic");

        // l ≥ 2, m=0: non-zero, finite
        for (int l = 2; l <= 4; ++l) {
            auto gt = eng.get_ang_grad_theta(l, 0);
            check(std::isfinite(gt[0]),
                  std::format("get_ang_grad_theta({},0) finite", l));
        }
    }
        ...
        for (int l = 0; l <= 4; ++l) {
            for (int m = -l; m <= l; ++m) {
                ...
                RealSphericalHarmonicsEngine eng_ref{std::span<const double>(&theta_base, 1),
                                                     std::span<const double>(&phi, 1), 4};
                double analytic = eng_ref.get_ang_grad_theta(l, m)[0];
                check(near(fd, analytic, 1e-6),
                      std::format("finite diff dY({},{})/dtheta = ang_grad_theta", l, m));
            }
        }
    }
    ...
    // ---------- theta = 0 edge: l=1,|m|≤1 finite, others zero ----------
    {
        ...
        for (int l = 0; l <= 4; ++l) {
            ...
            check(near(gt[0], 0.0, eps),
                  std::format("get_ang_grad_theta({},0) = 0 at θ=0", l));
        }
        ...
        // l=1, m=±1: known analytic values at θ=0
        {
            ...
            auto gt11 = eng.get_ang_grad_theta(1, 1);
            check(near(gt11[0], expected_11, 1e-12),
                  "get_ang_grad_theta(1,1) at θ=0 matches analytic limit");
            ...
            auto gt1m1 = eng.get_ang_grad_theta(1, -1);
        ...
        for (int l = 2; l <= 4; ++l) {
            for (int m = -l; m <= l; ++m) {
                if (std::abs(m) < 2) continue;
                auto gt = eng.get_ang_grad_theta(l, m);
                check(near(gt[0], 0.0, 1e-14),
                      std::format("get_ang_grad_theta({},{}) = 0 at θ=0 (|m|>=2)", l, m));
            }
        }
    }
    ...
    // ---------- Mode=None throws ----------
    {
        ...
        RealSphericalHarmonicsEngine eng{std::span<const double>(&theta, 1), std::span<const double>(&phi, 1),
                                   0, RealSphericalHarmonicsEngine::CacheMode::None};
        try {
            eng.get_ang_grad_theta(0, 0);
            check(false, "get_ang_grad_theta should throw in CacheMode::None");
        } catch (const std::runtime_error&) {
            check(true, "get_ang_grad_theta throws in CacheMode::None");
        }
    }
    ...
    // ---------- l > l_max_resident throws ----------
    {
        ...
        RealSphericalHarmonicsEngine eng{std::span<const double>(&theta, 1), std::span<const double>(&phi, 1), 2};
        try {
            eng.get_ang_grad_theta(3, 0);
            check(false, "get_ang_grad_theta should throw when l > l_max_resident");
        } catch (const std::runtime_error&) {
            check(true, "get_ang_grad_theta throws when l > l_max_resident");
        }
    }
}

auto test_ang_grad_phi_cache_lifecycle() -> void {
    std::println("\n=== get_ang_grad_phi() cache lifecycle ===");

    double eps = 1e-14;
    double theta_arr[] = {0.5, 0.7};
    double phi_arr[] = {0.3, 0.6};

    // ---------- setLMax expand after get_ang_grad_phi ----------
    {
        RealSphericalHarmonicsEngine eng{theta_arr, phi_arr, 2};
        // Fill cache for l_max=2
        auto gp20 = eng.get_ang_grad_phi(2, 1);
        double ref = gp20[1];   // second G-vector (θ=0.7)
        check(std::isfinite(ref), "ang_grad_phi(2,1) finite at l_max=2");

        // Expand and read — should work (cache repopulated lazily)
        eng.setLMax(4);
        auto gp31 = eng.get_ang_grad_phi(3, 1);
        check(std::isfinite(gp31[0]), "ang_grad_phi(3,1) works after expand");
        check(std::isfinite(gp31[1]), "ang_grad_phi(3,1) works after expand (point 2)");
    }

    // ---------- reinit → setLMax → get_ang_grad_phi ----------
    {
        RealSphericalHarmonicsEngine eng{theta_arr, phi_arr, 2};
        eng.get_ang_grad_phi(2, 1);   // populate cache

        // Reinit with same theta/phi but different array
        eng.reinit(theta_arr, phi_arr);
        eng.setLMax(2);
        auto gp = eng.get_ang_grad_phi(2, 1);
        check(std::isfinite(gp[0]), "ang_grad_phi works after reinit + setLMax");
    }
}

auto test_ang_grad_theta_cache_lifecycle() -> void {
    std::println("\n=== get_ang_grad_theta() cache lifecycle ===");

    double eps = 1e-14;
    double theta_arr[] = {0.5, 0.7};
    double phi_arr[] = {0.3, 0.6};

    // ---------- setLMax expand after get_ang_grad_theta ----------
    {
        RealSphericalHarmonicsEngine eng{theta_arr, phi_arr, 2};
        // Fill cache for l_max=2
        auto gt20 = eng.get_ang_grad_theta(2, 1);
        double ref = gt20[1];   // second G-vector (θ=0.7)
        check(std::isfinite(ref), "ang_grad_theta(2,1) finite at l_max=2");

        // Expand and read — should work (cache repopulated lazily)
        eng.setLMax(4);
        auto gt31 = eng.get_ang_grad_theta(3, 1);
        check(std::isfinite(gt31[0]), "ang_grad_theta(3,1) works after expand");
        check(std::isfinite(gt31[1]), "ang_grad_theta(3,1) works after expand (point 2)");
    }

    // ---------- reinit → setLMax → get_ang_grad_theta ----------
    {
        RealSphericalHarmonicsEngine eng{theta_arr, phi_arr, 2};
        eng.get_ang_grad_theta(2, 1);   // populate cache

        // Reinit with same theta/phi but different array
        eng.reinit(theta_arr, phi_arr);
        eng.setLMax(2);
        auto gt = eng.get_ang_grad_theta(2, 1);
        check(std::isfinite(gt[0]), "ang_grad_theta works after reinit + setLMax");
    }
}
...
auto test_reset_shrink_expand() -> void {
    ...
    // Pre-shrink ang_grad_phi cache references
    auto g_ref_21 = eng.get_ang_grad_phi(2, 1);
    auto g_ref_41 = eng.get_ang_grad_phi(l_max, 1);
    auto g_ref_4m1 = eng.get_ang_grad_phi(l_max, -1);
    // Pre-shrink ang_grad_theta cache references
    auto gtr_21 = eng.get_ang_grad_theta(2, 1);
    auto gtr_41 = eng.get_ang_grad_theta(l_max, 1);
    auto gtr_4m1 = eng.get_ang_grad_theta(l_max, -1);
    ...
    // ang_grad_phi cache preserved (inner vectors for m_abs≥1 retained)
    check(near(eng.get_ang_grad_phi(2, 1)[0], g_ref_21[0], eps),
          "ang_grad_phi(2,1) unchanged after shrink");
    // ang_grad_theta cache preserved
    check(near(eng.get_ang_grad_theta(2, 1)[0], gtr_21[0], eps),
          "ang_grad_theta(2,1) unchanged after shrink");
    ...
    // ang_grad_phi cache preserved through shrink/expand cycle
    check(near(eng.get_ang_grad_phi(2, 1)[0], g_ref_21[0], eps),
          "ang_grad_phi(2,1) matches pre-shrink after expand");
    check(near(eng.get_ang_grad_phi(l_max, 1)[0], g_ref_41[0], eps),
          "ang_grad_phi(l_max,1) matches pre-shrink after expand");
    check(near(eng.get_ang_grad_phi(l_max, -1)[0], g_ref_4m1[0], eps),
          "ang_grad_phi(l_max,-1) matches pre-shrink after expand");
    // ang_grad_theta cache preserved through shrink/expand cycle
    check(near(eng.get_ang_grad_theta(2, 1)[0], gtr_21[0], eps),
          "ang_grad_theta(2,1) matches pre-shrink after expand");
    check(near(eng.get_ang_grad_theta(l_max, 1)[0], gtr_41[0], eps),
          "ang_grad_theta(l_max,1) matches pre-shrink after expand");
    check(near(eng.get_ang_grad_theta(l_max, -1)[0], gtr_4m1[0], eps),
          "ang_grad_theta(l_max,-1) matches pre-shrink after expand");
}

auto test_data_ang_grad_phi() -> void {
    std::println("\n=== RealSphericalHarmonicsData with AngGradPhi ===");
    ...
    // ---------- Y_lm default construction still works ----------
    {
        ...
        check(near(y00[0], 1.0 / (2.0 * std::sqrt(std::numbers::pi)), eps),
              "[ang_grad_phi test] default Ylm construction still works");
    }

    // ---------- Construction with Quantity::AngGradPhi ----------
    {
        double theta = 0.7, phi = 0.3;
        RealSphericalHarmonicsEngine eng{std::span<const double>(&theta, 1),
                                         std::span<const double>(&phi, 1), 4};
        RealSphericalHarmonicsData data{RealSphericalHarmonicsData::Quantity::AngGradPhi};

        // m > 0: matches eng.get_ang_grad_phi()
        auto gp21 = eng.get_ang_grad_phi(2, 1);
        const auto& d_gp21 = data.get(eng, 2, 1);
        check(near(d_gp21[0], gp21[0], eps),
              "AngGradPhi Data get(2,1) matches Engine");

        // m < 0: matches eng.get_ang_grad_phi()
        auto gp2m1 = eng.get_ang_grad_phi(2, -1);
    ...
    // ---------- Cache hit: same address on second call ----------
    {
        ...
        check(&first == &second, "AngGradPhi Data cache hit: same address on second call");
    }
    ...
    // ---------- m = 0 returns zero for any l ----------
    {
        ...
        for (int l = 0; l <= 4; ++l) {
            ...
            check(near(gp[0], 0.0, eps),
                  std::format("AngGradPhi Data get({},0) = 0", l));
        }
    }
    ...
    // ---------- Multiple G-vectors ----------
    {
        ...
        check(gp.size() == 2, "AngGradPhi Data multi G-vector: size");

        auto direct = eng.get_ang_grad_phi(2, 1);
        check(near(gp[0], direct[0], eps), "AngGradPhi Data multi G-vector: point 0");
        check(near(gp[1], direct[1], eps), "AngGradPhi Data multi G-vector: point 1");
    }
    ...
    // ---------- Cache invalidated by reinit ----------
    {
        ...
        // reinit → setLMax → new ang_grad_phi values
        eng.reinit(theta_b, phi_b);
        eng.setLMax(4);

        auto ref = eng.get_ang_grad_phi(2, 1);
        const auto& cached = data.get(eng, 2, 1);
        check(near(cached[0], ref[0], eps),
              "AngGradPhi Data cache correct after reinit");
    }
    ...
    // ---------- Cache invalidated by setLMax ----------
    {
        ...
        auto ref = eng.get_ang_grad_phi(3, 2);
        check(near(after[0], ref[0], eps),
              "AngGradPhi Data get(3,2) correct after engine setLMax(5)");

        eng.setLMax(2);         // shrink
        // After revision change Data invalidates all entries, then refills
        // via filler_ → eng.get_ang_grad_phi().  (1,1) is within l_max=2, so
        // it produces a valid value rather than throwing.
        const auto& gp11 = data.get(eng, 1, 1);
        auto ref11 = eng.get_ang_grad_phi(1, 1);
        check(near(gp11[0], ref11[0], eps),
              "AngGradPhi Data get(1,1) correct after engine shrink to l_max=2");
    }
}

auto test_data_ang_grad_theta() -> void {
    std::println("\n=== RealSphericalHarmonicsData with AngGradTheta ===");
    ...
    // ---------- Y_lm default construction still works ----------
    {
        ...
        const auto& y00 = ylm_data.get(eng, 0, 0);
        check(near(y00[0], 1.0 / (2.0 * std::sqrt(std::numbers::pi)), eps),
              "[ang_grad_theta test] default Ylm construction still works");
    }

    // ---------- Construction with Quantity::AngGradTheta ----------
    {
        double theta = 0.7, phi = 0.3;
        RealSphericalHarmonicsEngine eng{std::span<const double>(&theta, 1),
                                         std::span<const double>(&phi, 1), 4};
        RealSphericalHarmonicsData data{RealSphericalHarmonicsData::Quantity::AngGradTheta};

        auto gt21 = eng.get_ang_grad_theta(2, 1);
        const auto& d_gt21 = data.get(eng, 2, 1);
        check(near(d_gt21[0], gt21[0], eps),
              "AngGradTheta Data get(2,1) matches Engine");

        auto gt2m1 = eng.get_ang_grad_theta(2, -1);
    ...
    // ---------- Cache hit: same address on second call ----------
    {
        ...
        const auto& first  = data.get(eng, 3, 2);
        const auto& second = data.get(eng, 3, 2);
        check(&first == &second, "AngGradTheta Data cache hit: same address on second call");
    }
    ...
    // ---------- m = 0 returns finite values (non-zero for l ≥ 1) ----------
    {
        ...
        check(near(gt00[0], 0.0, eps),
              "AngGradTheta Data get(0,0) = 0");

        // l ≥ 1: non-zero
        for (int l = 1; l <= 4; ++l) {
            const auto& gt = data.get(eng, l, 0);
            check(std::isfinite(gt[0]),
                  std::format("AngGradTheta Data get({},0) finite", l));
        }
    }
    ...
    // ---------- Multiple G-vectors ----------
    {
        ...
        check(gt.size() == 2, "AngGradTheta Data multi G-vector: size");

        auto direct = eng.get_ang_grad_theta(2, 1);
        check(near(gt[0], direct[0], eps), "AngGradTheta Data multi G-vector: point 0");
        check(near(gt[1], direct[1], eps), "AngGradTheta Data multi G-vector: point 1");
    }
    ...
    // ---------- Cache invalidated by reinit ----------
    {
        ...
        auto ref = eng.get_ang_grad_theta(2, 1);
        const auto& cached = data.get(eng, 2, 1);
        check(near(cached[0], ref[0], eps),
              "AngGradTheta Data cache correct after reinit");
    }
    ...
    // ---------- Cache invalidated by setLMax ----------
    {
        ...
        eng.setLMax(5);         // expand
        const auto& after = data.get(eng, 3, 2);
        auto ref = eng.get_ang_grad_theta(3, 2);
        check(near(after[0], ref[0], eps),
              "AngGradTheta Data get(3,2) correct after engine setLMax(5)");

        eng.setLMax(2);         // shrink
        const auto& gt11 = data.get(eng, 1, 1);
        auto ref11 = eng.get_ang_grad_theta(1, 1);
        check(near(gt11[0], ref11[0], eps),
              "AngGradTheta Data get(1,1) correct after engine shrink to l_max=2");
    }
}
...
auto main() -> int {
    try {
        ...
        test_ang_grad_phi();
        test_ang_grad_theta();
        test_ang_grad_phi_cache_lifecycle();
        test_ang_grad_theta_cache_lifecycle();
        test_get_beyond_lmax();
        test_reset_shrink_expand();
        test_data_ang_grad_phi();
        test_data_ang_grad_theta();
