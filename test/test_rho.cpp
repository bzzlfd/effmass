...
auto testVR() -> void {
    ...
    if (m.nnodes != 1) {
        throw std::runtime_error("VR nnodes mismatch: expected 1, got " + std::to_string(m.nnodes));
    }
    if (m.nstate != 1) {
    ...
    if (std::abs(vr[0, 0, 0] - 0.093148) > 1e-6) {
        throw std::runtime_error("VR[0,0,0] mismatch: expected ~0.093148, got " +
            std::to_string(vr[0, 0, 0]));
    }

    // Spot-check some other values
    if (std::abs(vr[0, 0, 1] - 0.111383) > 1e-6) {
        throw std::runtime_error("VR[0,0,1] mismatch: expected ~0.111383, got " +
            std::to_string(vr[0, 0, 1]));
    }
    if (std::abs(vr[0, 0, 2] - 0.099576) > 1e-6) {
        throw std::runtime_error("VR[0,0,2] mismatch: expected ~0.099576, got " +
...
auto testRHO() -> void {
    ...
    if (m.nnodes != 1) {
        throw std::runtime_error("RHO nnodes mismatch: expected 1, got " + std::to_string(m.nnodes));
    }
    if (m.nstate != 1) {
        throw std::runtime_error("RHO nstate mismatch: expected 1, got " + std::to_string(m.nstate));
    }

    // RHO has different values from VR: first value ~0.010028
    if (std::abs(rho[0, 0, 0] - 0.010028) > 1e-6) {
        throw std::runtime_error("RHO[0,0,0] mismatch: expected ~0.010028, got " +
