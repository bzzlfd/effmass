...
auto main() -> int {
    try {
        EIGEN eig("test/test_io/OUT.EIGEN");
        ...
        if (std::abs(eig.eigenvalue[0, 0] - (-94.56160299738882)) > 1e-10) {
            throw std::runtime_error("eigenvalue[0, 0] mismatch");
        }
        if (std::abs(eig.eigenvalue[1, 0] - (-94.55843559389352)) > 1e-10) {
            throw std::runtime_error("eigenvalue[1, 0] mismatch");
        }
        if (std::abs(eig.eigenvalue[2, 0] - (-56.10710797636899)) > 1e-10) {
            throw std::runtime_error("eigenvalue[2, 0] mismatch");
        }
        if (std::abs(eig.eigenvalue[23, 9] - 19.734772719933048) > 1e-10) {
            throw std::runtime_error("eigenvalue[23, 9] mismatch");
        }
        if (std::abs(eig.eigenvalue[24, 9] - 22.390937731460543) > 1e-10) {
            throw std::runtime_error("eigenvalue[24, 9] mismatch");
        }
        if (std::abs(eig.eigenvalue[25, 9] - 22.390939925382884) > 1e-10) {
            throw std::runtime_error("eigenvalue[25, 9] mismatch");
        }

        // 3-arg access [iband, ikpt, ispin]
        if (std::abs(eig.eigenvalue[0, 0, 0] - (-94.56160299738882)) > 1e-10) {
            throw std::runtime_error("eigenvalue[0, 0, 0] mismatch");
        }
        if (std::abs(eig.eigenvalue[25, 9, 0] - 22.390939925382884) > 1e-10) {
