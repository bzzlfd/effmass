...
public:
    ...
    std::vector<vector3d<double>> frac_coord;   // fractional coordinates

    int ntyp{};                     // number of distinct species types

    // --- iteration views ---
    // (see dedicated section below for usage)

    auto eachType() const -> TypeView;
    auto eachType(std::span<const int> order) const -> TypeOrderView;
    auto eachTypeAtom(int ityp) const -> TypeAtomView;
    auto eachSpecie() const -> SpecieView;

private:
    std::vector<int> typeGroup_z_;           // Z per type, ascending
    std::vector<int> typeGroup_count_;       // atom count per type
    std::vector<int> atom_type_idx_;         // type index per atom
    std::vector<int> typeSort_order_;        // permutation: grouped → original index
};
```
...
4. **species 分析**：构造中解析完成后调用 `analyzeSpecies()`。类型维度数据和原子→类型映射为私有成员，通过视图访问。
   
   数据存储选型上，采用**分离的 vector**（`species`、`frac_coord`）而非 `vector<Atom>` 结构体数组。坐标使用 `vector3d<double>` 统一表示。利弊：
   - 分离 vector 允许调用方直接传递单个数组（如 `species.data()`），无需先拆包
   - 只关心 species 时无需附带位置数据
   - 排序需要额外维护一个排列数组，不能直接对容器 `sort`
   - 若改为结构体，排序更直接但失去扁平数组的灵活性。当前项目倾向于扁平数组风格。
   - `vector3d<double>` 替代三个独立的 `x/y/z` 向量，减少参数传递数量，且 `vector3d` 提供 `x/y/z` 成员访问和算术运算支持。
...
ATOM 提供四种 range-for 可遍历的视图，用于访问物种类型和原子数据，统一用 `auto&&` 接收元素（详见下方「为什么用 `auto&&`」）：

```cpp
// TypeView：遍历物种类型（默认按 Z 升序）
for (auto&& t : atom.eachType())          // ATOM::TypeDatum{z, count, ityp}
    std::println("Z = {}, count = {}", t.z, t.count);

// TypeOrderView：按自定义顺序遍历物种类型
for (auto&& t : atom.eachType({1, 0}))    // ATOM::TypeDatum{z, count, ityp}
    std::println("Z = {}, count = {}", t.z, t.count);

// TypeAtomView：遍历指定类型的所有原子（按 Z 分组后的顺序）
for (auto&& a : atom.eachTypeAtom(ityp))  // ATOM::AtomDatum{species, coord}
    std::println("species = {}, frac = ({}, {}, {})", a.species, a.coord.x, a.coord.y, a.coord.z);

// SpecieView：按原始读取顺序遍历所有原子
for (auto&& a : atom.eachSpecie())        // ATOM::AtomDatum{species, coord}
    std::println("{}", a.species);
```

#### 为什么用 `auto&&`

`operator*()` 按值返回 `TypeDatum` 或 `AtomDatum`，产生临时对象（右值）：
...
四个视图类均定义为 `ATOM` 的**嵌套类**（随 `export class ATOM` 自动导出），每个视图内部嵌套自己的 `Iterator` 类：

```
ATOM
├── TypeView          — eachType() 的返回类型
│   └── Iterator      — *it → TypeDatum{z, count, ityp}
├── TypeOrderView     — eachType(order) 的返回类型
│   └── Iterator      — *it → TypeDatum{z, count, ityp}，按 order 顺序
├── TypeAtomView      — eachTypeAtom(ityp) 的返回类型
│   └── Iterator      — *it → AtomDatum{species, coord}
└── SpecieView        — eachSpecie() 的返回类型
    └── Iterator      — *it → AtomDatum{species, coord}，原始顺序
```
...
**不是**让 `TypeView` 能访问 `ATOM` 的私有成员——`ATOM` 的 `typeGroup_z_`、`typeGroup_count_` 等是私有的，嵌套类自然有访问权限。而是**让 `ATOM` 能构造 `TypeView`**：把构造设为 `private` 防止用户绕过 `eachType()` 直接实例化视图。
...
```
每个 atom.eachType() + eachTypeAtom 组合  →  所有原子恰好一次，按 Z 分组
