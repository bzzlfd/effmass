  ...
  SUBROUTINE write_upf_v2(u, upf)
    ...
    TYPE(atompp),INTENT(IN) :: upf ! the pseudo data
    !
  ...
  CONTAINS
    ...
    use upf_module
      ...
      !
      WRITE(u, '(4x,a)') TRIM(CHECK(upf%generated))
      WRITE(u, '(4x,a)') &
      ...
      WRITE(u, '(4x,a2,2a3,a6,2a11,1a13)') &
           "nl"," pn", "l", "occ", "Rcut", "Rcut US", "E pseu"
      DO nb = 1, upf%nwfc
         IF(upf%oc(nb) >= 0._8) THEN
      ...
      IF(TRIM(upf%comment) /= ' ') THEN
         WRITE(u, '(4x,"Comment:",/,4x,a)') TRIM(CHECK(upf%comment))
      END IF
      !
    ...
    use upf_module
      ...
      CALL iotk_write_attr(attr, 'is_paw',         .false.,      newline=.true.)
      ...
      !EMINE
      CALL iotk_write_attr(attr, 'has_gipaw',      .false.,  newline=.true.)
      CALL iotk_write_attr(attr, 'paw_as_gipaw',   .false.,  newline=.true.)
      !
    ...
    use upf_module
      ...
      DO nb = 1,upf%nbeta
         ...
         IF ( upf%has_so ) THEN
            ...
            IF ( l > 0 .AND. ABS (upf%jjj(nb)-l-0.5_8) < 0.001_8) ind = 2
         ENDIF
